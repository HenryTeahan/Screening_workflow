...
def charge_is_OK(BO, AC, charge, DU, atomic_valence_electrons, atoms, valences,
                 allow_charged_fragments=True, allow_carbenes=True):
    # total charge
    Q = 0
...
def BO_is_OK(BO, AC, charge, DU, atomic_valence_electrons, atoms, valences,
    ...
    check_charge = charge_is_OK(BO, AC, charge, DU, atomic_valence_electrons, atoms, valences,
                                allow_charged_fragments, allow_carbenes=True)
...
def AC2BO(AC, atoms, charge, allow_charged_fragments=True, use_graph=True,
          allow_carbenes=True, bridging_hydrogens=[]):
    """
    ...
    O_valences = [v_list for v_list, atomicNum in zip(valences_list_of_lists, atoms) if atomicNum == 8]
    N_valences = [v_list for v_list, atomicNum in zip(valences_list_of_lists, atoms) if atomicNum == 7]
    C_valences = [v_list for v_list, atomicNum in zip(valences_list_of_lists, atoms) if atomicNum == 6]
    P_valences = [v_list for v_list, atomicNum in zip(valences_list_of_lists, atoms) if atomicNum == 15]
    S_valences = [v_list for v_list, atomicNum in zip(valences_list_of_lists, atoms) if atomicNum == 16]
    ...
    for valence_list in valences_list:
        ...
        order_idx = order_dict[(tuple(O_sum), tuple(N_sum), tuple(C_sum), tuple(P_sum), tuple(S_sum))]
        valence_order_list.append(order_idx)


    sorted_valences_list = [y for x,y in sorted(zip(valence_order_list, list(itertools.product(*valences_list_of_lists))))]
    ...
    for valences in sorted_valences_list:#valences_list:
        ...
        if check_len:
            check_bo = BO_is_OK(AC, AC, charge, DU_from_AC,
                ...
                allow_charged_fragments=allow_charged_fragments,allow_carbenes=allow_carbenes)
        else:
            check_bo = None
...
def AC2mol(mol, AC, atoms, charge, allow_charged_fragments=True,
        use_graph=True, use_atom_maps=True, allow_carbenes=True,
        bridging_hydrogens=[]):
    """
    """
    ...
    BO, atomic_valence_electrons = AC2BO(
        ...
        use_graph=use_graph,
        allow_carbenes=allow_carbenes,
        bridging_hydrogens=bridging_hydrogens)
...
def xyz2AC_obabel(atoms, xyz, tolerance=0.45):
    ...
    for i in range(num_atoms):
        ...
        for j in range(i + 1, num_atoms):
            ...
            if dMat[i, j] <= Rcov_i + Rcov_j + tolerance:
                AC[i, j] = 1
                AC[j, i] = 1
    #check for special boron case: if two borons and these share a neigbouring
    #Hydrogen -> update this hydrogen valence list to [2] 
    bridging_hydrogens = []
    if atoms.count(5) >= 2:
        print("two borons detected")
        for i in range(num_atoms):
            a = mol.GetAtomWithIdx(i)
            if a.GetAtomicNum() == 1:
                neighbors_idx = np.nonzero(AC[i,:])[0]
                neighbor_atomnums = [atoms[j] for j in neighbors_idx]
                #print(neighbor_atomnums)
                if neighbor_atomnums.count(5) == 2:
                    bridging_hydrogens.append(i)
    ...
    for i in range(num_atoms):
        ...
        if i in bridging_hydrogens:
            ...
            max_atom_valence = max(atomic_valence[a_i.GetAtomicNum()])

        while N_con > max_atom_valence:
            print("removing longest bond")
            AC = remove_weakest_bond(mol, i, AC, dMat, pt)
            N_con = np.sum(AC[i,:])
    ...
    return AC, mol, bridging_hydrogens
...
def xyz2mol(atoms, coordinates,
    ...
    AC_output = xyz2AC(atoms, coordinates, charge, use_huckel=use_huckel,
                       use_obabel=use_obabel)
    if len(AC_output) == 3:
        AC, mol, bridging_hydrogens = AC_output
    else:
        AC, mol = AC_output
        bridging_hydrogens = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-obabel',
        action="store_true",
